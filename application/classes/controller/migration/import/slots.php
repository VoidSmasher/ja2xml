...
/**
 * Class: Controller_Migration_Import_Slots
 * User: legion
 * Date: 24.10.17
 * Time: 9:17
 */
class Controller_Migration_Import_Slots extends Controller_Migration_Template {

	protected $migration_title = 'Импорт данных из AttachmentSlots.xml';
	protected $migration_description = [];

	const FILE_PATH = 'ja2/7609/items/AttachmentSlots.xml';
	...
	public function action_json_process() {
		...
		try {
			foreach ($xml->ATTACHMENTSLOT as $slot) {
				$model = Core_Slot::factory()->get_builder()
					->where('uiSlotIndex', '=', $slot['uiSlotIndex'])
					->limit(1)
					->select();
...
} // End Controller_Migration_Import_Slots