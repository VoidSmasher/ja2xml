...
/**
 * Class: Controller_Admin_Choices_Gun
 * User: legion
 * Date: 27.03.2021
 * Time: 15:50
 */
class Controller_Admin_Choices_Gun extends Controller_Admin_Template {

	public function action_index() {
		$builder = Core_Choices_Gun_Mod::factory()->preset_for_admin()->get_builder()
			->join('items', 'left')->on('items.uiIndex', '=', 'choices_gun_mod.uiIndex')
			->join('items_mod', 'left')->on('items_mod.uiIndex', '=', 'choices_gun_mod.uiIndex')
			->select_column('choices_gun_mod.*')
		...
		$filter = Force_Filter::factory(array(
			Force_Filter_Select::factory('npc', 'NPC', Core_Choices_Gun::get_combo_filter(), false),
			Force_Filter_Select::factory('list', 'List', Core_Choices_Gun::get_lists())
				->where('list_index'),
		))->apply($builder);

		Core_Choices_Gun::apply_combo_filter($builder, $filter);
		...
		$list->apply($builder, null, false)
			->each(function (Model_Choices_Gun_Mod $model) {
				$row = Force_List_Row::factory();

				Core_Item::row_image($model, $row, 'weapon_image');
//				$model->format('list', Core_Choices_Gun::get_list_name($model->list));
	...
	public function action_add() {
		$model = Core_Choices_Gun_Mod::factory()->create();
		$this->_form($model);
	}

	public function action_edit() {
		$model = Core_Choices_Gun_Mod::factory()->preset_for_admin()->on_error_throw_404()->request_id()->get_one();
		$this->_form($model);
	}
	...
	public function action_delete() {
		if (Core_Choices_Gun_Mod::factory()->preset_for_admin()->on_error_throw_404()->request_id()->delete()) {
			$this->_back_to_index();
		}
	}
