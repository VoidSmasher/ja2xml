...
/**
 * Class: Force_Form_Slider
 * User: legion
 * Date: 24.07.14
 * Time: 19:42
 */
class Force_Form_Slider extends Force_Form_Control {
	...
	public function render() {
		...
		$this->_view_data = [
			...
			'path' => Kohana::$config->load('images.' . $this->_image_type_to_display . '.dir'),
		];
	...
	public function apply_value_before_save(Force_Form_Core $form, $new_value = null, $old_value = null) {
		...
		$remove = $form->get_value($this->get_name_remove_image(), false);

		if( is_array($remove) && !empty($remove) ){
			foreach( $remove as $remove_filename) {
				Helper_Image::remove_image($remove_filename, $image_type);
			}

			if( is_array($old_value) && !empty($old_value) ){
				$new_old_value = array();
				for($i=0;$i<count($old_value);$i++){
					if( !in_array($old_value[$i],$remove) ){
						$new_old_value[] = $old_value[$i];
					}
				}
		...
		if (array_key_exists($this->get_name(), $_FILES)) {
			...
			$FILES = $_FILES[$this->get_name()];
			$files_to_upload = array();
			foreach( $FILES as $field => $values ){
				foreach( $values as $i => $field_value ){
					if( !array_key_exists($i,$files_to_upload) ) {
						$files_to_upload[$i] = array();
					}

					$files_to_upload[$i][$field] = $field_value;
				}
			}
			...
			foreach( $files_to_upload as $file_to_upload ){
				if( $new_filename = Helper_Image::upload($file_to_upload, false, $image_type, $this->get_name(), $this->get_label(), $required) ) {
					$new_filenames[] = $new_filename;
				}
			}

			if( is_array($new_filenames) && !empty($new_filenames) ){
				if( !is_array($old_value) ) { $old_value = array(); }
				foreach( $new_filenames as $new_filename ){
					$old_value[] = (string)$new_filename;
				}
			}
		}
