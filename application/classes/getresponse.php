...
class Getresponse
{
	...
	*/
	 ...
	 * @return void
	 */
	public function __construct($print_errors = true)
	{

        $this->apiKey = Kohana::$config->load('getresponse.key');

		if(!extension_loaded('curl')) trigger_error('GetResponsePHP requires PHP cURL', E_USER_ERROR);
		if(is_null(  $this->apiKey)) trigger_error('API key must be supplied', E_USER_ERROR);
